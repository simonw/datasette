...
from datasette import Request, Response, __version__
...
from .base import DataView, DatasetteError, ureg, BaseView
...
async def filter_args(request: Request) -> List[Tuple[str, str]]:
...
class TableView(BaseView):
    ...
    async def data(self, request):
        ...
        registry = Registry(filter_args, database, table_name)

        breakpoint()
        resolved = await registry.resolve_multi(
            ["filter_args", "database", "table_name"],
            results=dict(datasette=self.ds, request=request),
        )
        print(resolved)
        ...
        canned_query = await self.ds.get_canned_query(
            database_name, table, request.actor
        )
        if canned_query:
        ...
        await self.ds.ensure_permissions(
            request.actor,
            [
                ("view-table", (database_name, table)),
        ...
        private = not await self.ds.permission_allowed(
            None, "view-table", (database_name, table), default=True
        )
        ...
        table_metadata = self.ds.table_metadata(database_name, table)
        units = table_metadata.get("units", {})

        filter_args_ = filter_args(request)

        # Build where clauses from query string arguments
        filters = Filters(sorted(filter_args_), units, ureg)
        ...
        expanded_columns = []
        expandable_columns = await self.expandable_columns(database_name, table)
        columns_to_expand = None
        try:
            all_labels = value_as_boolean(request.args.get("_labels", ""))
        except ValueError:
            # TODO: Should be True for HTML views
            all_labels = False
            ...
            for fk, _ in expandable_columns:
                ...
                expanded_labels.update(
                    await self.ds.expand_foreign_keys(
                        database_name, table, column, values
                    )
                )
            if expanded_labels:
                # Rewrite the rows
        ...
        async def extra_template():
            ...
            d = {
                ...
                "custom_table_templates": [
                    f"_table-{to_css_class(database_name)}-{to_css_class(table)}.html",
                    f"_table-table-{to_css_class(database_name)}-{to_css_class(table)}.html",
                    "_table.html",
                ],
                "metadata": metadata,
        ...
        return (
            ...
            extra_template,
            (
                f"table-{to_css_class(database_name)}-{to_css_class(table)}.html",
                "table.html",
            ),
        )
