...
- While the request span is recording, responses now carry its trace context in a W3C ``traceresponse`` header and a ``Server-Timing: traceparent;desc="..."`` entry, so a client or page-load JavaScript can find the trace for the request it just made. ``traceresponse`` is appended to ``Access-Control-Expose-Headers`` when CORS is enabled. See :ref:`internals_telemetry_requests`. (:issue:`1730`)
- Each call to a plugin hook implementation now emits a ``datasette.hook`` OpenTelemetry span carrying the hook and plugin name, so a trace shows which plugin's hook took how long. For an ``async def`` implementation the span covers the ``await``, unlike ``DATASETTE_TRACE_PLUGINS``, which reports those as taking no time. The high-volume ``render_cell`` and ``permission_resources_sql`` hooks, and the app-construction hooks ``register_routes`` and ``asgi_wrapper``, are not traced. See :ref:`internals_telemetry`. (:issue:`1730`, :issue:`2274`)
- Permission checks now emit OpenTelemetry spans, so a trace shows how much of a request went to permission resolution: ``datasette.permission.check`` for each call to ``datasette.allowed()``, ``allowed_many()``, ``ensure_permission()`` or ``check_visibility()``, and ``datasette.permission.resources`` for ``allowed_resources()`` and ``allowed_resources_sql()``. They carry the action, the resource's parent and child and, where there is a single answer, the verdict - never the actor. An API that delegates to another emits one span, not two. See :ref:`internals_telemetry`. (:issue:`1730`)
- Template rendering now emits a ``datasette.render_template`` OpenTelemetry span per HTML page, carrying the name of the template that was actually selected in ``datasette.template.name``. It covers building the template context - so ``extra_template_vars``, ``extra_body_script`` and asset URL hook spans nest inside it - as well as the Jinja render. Error pages get one too. See :ref:`internals_telemetry`. (:issue:`1730`)
- Table page facets now emit OpenTelemetry spans: ``datasette.facet`` for each facet type with facets requested, carrying ``datasette.facet.type``, the faceted ``datasette.facet.columns`` and any ``datasette.facet.timed_out_columns`` - a facet hitting :ref:`setting_facet_time_limit_ms` is not marked as an error - with the facet SQL nested inside, and a single ``datasette.facet.suggest`` span covering suggested-facet discovery with a ``datasette.facet.suggestion_count``. See :ref:`internals_telemetry`. (:issue:`1730`)
- CSV exports now emit a ``datasette.csv`` OpenTelemetry span covering the writing of the response body, including every page of a ``?_stream=on`` export - whose ``db.query`` spans nest inside it - carrying ``datasette.csv.stream``, ``datasette.csv.rows_written`` and ``datasette.csv.truncated``, which is true when :ref:`setting_max_csv_mb` cut the export short. Row data is never recorded. See :ref:`internals_telemetry`. (:issue:`1730`)
