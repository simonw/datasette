...
def test_custom_sql(app_client):
    ...
    assert [{"content": "hello"}, {"content": "world"}, {"content": ""}] == data["rows"]
...
def test_allow_sql_off():
    for client in app_client(config={"allow_sql": False}):
        assert 400 == client.get("/fixtures.json?sql=select+sleep(0.01)").status
...
def test_jsono_redirects_to_shape_objects(app_client):
    ...
    response = app_client.get(response_1.headers["Location"], allow_redirects=False)
...
def test_table_shape_arrays(app_client):
    ...
    assert [["1", "hello"], ["2", "world"], ["3", ""]] == response.json["rows"]
...
def test_table_with_reserved_word_name(app_client):
    ...
    data = response.json
    assert data["rows"] == [
        {"rowid": 1, "group": "group", "having": "having", "and": "and"}
    ]
...
def test_suggested_facets(app_client):
    assert len(app_client.get("/fixtures/facetable.json").json["suggested_facets"]) > 0


def test_allow_facet_off():
    for client in app_client(config={"allow_facet": False}):
...
def test_suggest_facets_off():
    for client in app_client(config={"suggest_facets": False}):
