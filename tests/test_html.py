...
from .fixtures import app_client, app_client_shorter_time_limit  # noqa
import pytest
import re
import urllib.parse
...
def test_row(app_client):
    response = app_client.get("/fixtures/simple_primary_key/1", allow_redirects=False)
    assert response.status == 302
...
def test_sort_by_desc_redirects(app_client):
    path_base = app_client.get("/fixtures/sortable", allow_redirects=False).headers[
        "Location"
    ]
    path = (
        path_base
...
def test_table_html_simple_primary_key(app_client):
    ...
    for expected_col, th in zip(("content",), ths[1:]):
        ...
        assert a["href"].endswith("/simple_primary_key?_sort={}".format(expected_col))
...
def test_view_html(app_client):
    ...
    expected = [
        ...
        ['<td class="col-content"></td>', '<td class="col-upper_content"></td>'],
    ]
    assert expected == [
        [str(td) for td in tr.select("td")] for tr in table.select("tbody tr")
    ]
...
def test_allow_download_off():
    for client in app_client(config={"allow_download": False}):
