...
def plugin_allow_all_for_user(user: str) -> PluginSQL:
    return PluginSQL(
        "allow_all",
        """
        SELECT NULL AS parent, NULL AS child, 1 AS allow, 'global allow for ' || :user AS reason
        WHERE :actor = :user
        """,
        {"user": user},
    )
...
def plugin_deny_specific_table(user: str, parent: str, child: str) -> PluginSQL:
    return PluginSQL(
        "deny_specific_table",
        """
        SELECT :parent AS parent, :child AS child, 0 AS allow,
               'deny ' || :parent || '/' || :child || ' for ' || :user AS reason
        WHERE :actor = :user
        """,
...
def plugin_org_policy_deny_parent(parent: str) -> PluginSQL:
    return PluginSQL(
        "org_policy_parent_deny",
        """
        SELECT :parent AS parent, NULL AS child, 0 AS allow,
               'org policy: parent ' || :parent || ' denied' AS reason
        """,
        {"parent": parent},
    )


def plugin_allow_parent_for_user(user: str, parent: str) -> PluginSQL:
    return PluginSQL(
        "allow_parent",
        """
        SELECT :parent AS parent, NULL AS child, 1 AS allow,
               'allow full parent for ' || :user AS reason
        WHERE :actor = :user
        """,
...
def plugin_child_allow_for_user(user: str, parent: str, child: str) -> PluginSQL:
    return PluginSQL(
        "allow_child",
        """
        SELECT :parent AS parent, :child AS child, 1 AS allow,
               'allow child for ' || :user AS reason
        WHERE :actor = :user
        """,
        {"parent": parent, "child": child, "user": user},
    )
...
def plugin_root_deny_for_all() -> PluginSQL:
    return PluginSQL(
        "root_deny",
        """
...
def plugin_conflicting_same_child_rules(user: str, parent: str, child: str) -> List[PluginSQL]:
    return [
        PluginSQL(
            "conflict_child_allow",
            """
            SELECT :parent AS parent, :child AS child, 1 AS allow,
                   'team grant at child for ' || :user AS reason
            WHERE :actor = :user
            """,
            {"parent": parent, "child": child, "user": user},
        ),
        PluginSQL(
            ...
            """
            SELECT :parent AS parent, :child AS child, 0 AS allow,
                   'exception deny at child for ' || :user AS reason
            WHERE :actor = :user
            """,
            {"parent": parent, "child": child, "user": user},
        ),
...
TABLE_CANDIDATES_SQL = "SELECT database_name AS parent, table_name AS child FROM catalog_tables"
PARENT_CANDIDATES_SQL = "SELECT database_name AS parent, NULL AS child FROM catalog_databases"
...
def res_allowed(rows, parent=None):
    return sorted(
        r["resource"] for r in rows if r["allow"] == 1 and (parent is None or r["parent"] == parent)
    )


def res_denied(rows, parent=None):
    return sorted(
        r["resource"] for r in rows if r["allow"] == 0 and (parent is None or r["parent"] == parent)
    )
...
async def test_alice_global_allow_with_specific_denies_catalog(db):
    ...
    rows = await resolve_permissions_from_catalog(
        db, "alice", plugins, TABLE_CANDIDATES_SQL, implicit_deny=True
    )
...
async def test_carol_parent_allow_but_child_conflict_deny_wins_catalog(db):
    ...
    rows = await resolve_permissions_from_catalog(
        db, "carol", plugins, TABLE_CANDIDATES_SQL, implicit_deny=True
    )
...
async def test_specificity_child_allow_overrides_parent_deny_catalog(db):
    ...
    plugins = [
        ...
        plugin_child_allow_for_user("alice", "analytics", "table02"),  # child allow beats parent deny
    ]
    rows = await resolve_permissions_from_catalog(
        db, "alice", plugins, TABLE_CANDIDATES_SQL, implicit_deny=True
    )
    ...
    assert all(
        (r["parent"] != "analytics" or r["child"] == "table02" or r["allow"] == 0) for r in rows
    )
...
async def test_root_deny_all_but_parent_allow_rescues_specific_parent_catalog(db):
    ...
    plugins = [
        ...
        plugin_allow_parent_for_user("bob", "accounting"),  # parent allow (more specific)
    ]
    rows = await resolve_permissions_from_catalog(
        db, "bob", plugins, TABLE_CANDIDATES_SQL, implicit_deny=True
    )
    for r in rows:
...
async def test_parent_scoped_candidates(db):
    ...
    rows = await resolve_permissions_from_catalog(
        db, "carol", plugins, PARENT_CANDIDATES_SQL, implicit_deny=True
    )
...
async def test_implicit_deny_behavior(db):
    ...
    rows = await resolve_permissions_from_catalog(
        db, "erin", plugins, TABLE_CANDIDATES_SQL, implicit_deny=True
    )
    ...
    rows2 = await resolve_permissions_from_catalog(
        db, "erin", plugins, TABLE_CANDIDATES_SQL, implicit_deny=False
    )
...
async def test_candidate_filters_via_params(db):
    ...
    await db.execute_write("UPDATE catalog_databases SET is_memory=1 WHERE database_name='hr'")
    await db.execute_write("UPDATE catalog_databases SET schema_version=2 WHERE database_name='analytics'")
    ...
    plugins = [
        ...
        plugin_allow_parent_for_user("dev", "analytics"),  # analytics rescued if included by candidates
    ]
